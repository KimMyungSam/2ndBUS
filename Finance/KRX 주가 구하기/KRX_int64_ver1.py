            ...
            for i in range(0,len(df)):
                ...
                for var in varlist:
                    if isinstance( var, np.int64 ):
                        ...
                        values.append(var)
                try:
                    sql = """INSERT INTO krx_stock_price (종목코드,종목명,현재가,대비,`등락률(%)`,`거래량(주)`,거래대금,시가,고가,저가,시가총액,`시가총액비중(%)`,상장주식수,외국인보유수량,`외국인 지분율(%)`,date)VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
                except:
                    print ('데이터 오류=',values)
